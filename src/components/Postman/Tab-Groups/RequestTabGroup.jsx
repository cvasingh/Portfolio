...
import JsonEditorPane from '../Panes/JsonEditorPane';
...
export default function RequestTabGroup({
  ...
  return (
    <Tabs forceRenderTabPanel selectedTabClassName="border-b-2 text-orange-600">
      <TabList className="flex mt-5 border border-gray-300 rounded-t-lg">
        {requestTabs.map((tab, i) => (
          <Tab className="mr-3 py-2 px-4 border-orange-400 focus:outline-none 
                          hover:text-orange-500 cursor-pointer" key={`${tab.slug} + ${i}`}>
            {tab.title}
          </Tab>
        ))}
      </TabList>
      ...
      {requestTabs.map((tab, i) => (
        <TabPanel className="react-tabs__tab-panel px-4 py-4 rounded-b-lg border border-t-0 border-gray-300" key={`${tab.slug} + ${i}`}>
          <tab.panel
